from ATypeRank_models_add_neg_multiple import *
...
class Trainer:
    ...
    def reload(self):
        ...
        state = torch.load(state_dict_file, map_location=lambda storage, loc: storage.cuda())
    ...
    def write_training_log(self, data, epoch):
        # 分别记录每个损失值，使用不同的标签
        self.writer.add_scalar('Training_Loss/Loss', data['Loss'], epoch)
        self.writer.add_scalar('Training_Loss/loss_r_aug', data['loss_r_aug'], epoch)
        self.writer.add_scalar('Training_Loss/cl_loss', data['cl_loss'], epoch)
        logging.info("Epoch: {}\tLoss: {:.3f}\tloss_r_aug: {:.3f}\tcl_loss: {:.3f}\r".format(
            epoch, data['Loss'], data['loss_r_aug'], data['cl_loss']))
    ...
    def logging_training_data(self, data, epoch):
        # 只记录评估指标（MRR、Hits@10等），移除对损失键的访问
    ...
    def do_one_step(self, task, iseval=False, curr_rel='',epoch_num = int):
        loss, p_score, n_score = 0, 0, 0
        if not iseval:
            self.optimizer.zero_grad()
            p_score, n_score, loss_r_aug, cl_loss = self.metaR(task, iseval, curr_rel, epoch_num)
            y = torch.Tensor([1]).to(self.device)
            y = y.unsqueeze(0).expand(p_score.shape)
            loss = self.metaR.loss_func(p_score, n_score, y) + self.r_aug_w * (loss_r_aug + cl_loss)
            loss.backward(retain_graph=True)  
            self.optimizer.step()
        elif curr_rel != '':
            p_score_raw, n_score_raw, loss_r_aug, cl_loss = self.metaR(task, iseval, curr_rel, epoch_num)
            ...
            y = y.unsqueeze(0).expand(p_score.shape)
            loss = self.metaR.loss_func(p_score, n_score, y) + self.r_aug_w * (loss_r_aug + cl_loss)
        return loss, loss_r_aug, cl_loss, p_score, n_score
    ...
    def train(self):
        ...
        for e in range(self.training_load_epoch, self.epoch):
            ...
            loss, loss_r_aug, cl_loss, _, _ = self.do_one_step(train_task, iseval=False, curr_rel=curr_rel, epoch_num = e)
            # print the loss on specific epoch
            if e % self.print_epoch == 0:
                loss_num = loss.item()
                loss_r_aug_num = loss_r_aug.item()
                cl_loss_num = cl_loss.item()
                self.write_training_log({'Loss': loss_num,'loss_r_aug': loss_r_aug_num,'cl_loss': cl_loss_num}, e)
                print("Epoch: {}\tLoss: {:.4f}\tloss_r_aug: {:.4f}\tcl_loss: {:.4f}".format(e, loss_num,loss_r_aug_num,cl_loss_num))
    ...
    def eval(self, istest=False, epoch=None):
        ...
        while True:
            ...
            _, _, _, p_score, n_score = self.do_one_step(eval_task, iseval=True, curr_rel=curr_rel, epoch_num = epoch)
    ...
    def eval_by_relation(self, istest=False, epoch=None):
        ...
        for rel in data_loader.all_rels:
            ...
            while True:
                ...
                _, _, _, p_score, n_score = self.do_one_step(eval_task, iseval=True, curr_rel=rel, epoch_num = epoch)
                x = torch.cat([n_score, p_score], 1).squeeze()
